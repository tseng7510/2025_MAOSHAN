...
(function () {
  $('.insidePage .subMenu button').on('click', function () {
    if (window.innerWidth <= setRWDWidth) {
      $('.insidePage .subMenu ul').slideToggle(300);
    }
  });
})();
// faq
...
// Parallax scroll effect for elements with .acBox classes
$(function () {
  const parallaxElements = $('.acBoxDown, .acBoxUp, .acBoxLeft');

  function updateParallaxEffect() {
    const windowWidth = $(window).width();

    // if (windowWidth <= 1000) {
    //   // On mobile, ensure transformations are removed.
    ...
    parallaxElements.each(function () {
      const $this = $(this);
      const box_t = $this.offset().top;
      const parallaxValue = windowScrollTop + windowHeight / 3 - box_t;

      if ($this.hasClass('acBoxDown')) {
        if (parallaxValue > 0) {
          gsap.to(this, { y: parallaxValue / 30, duration: 1, ease: 'none', overwrite: 'auto' });
        } else {
          gsap.to(this, { y: 0, duration: 1, ease: 'none', overwrite: 'auto' });
        }
      } else if ($this.hasClass('acBoxUp')) {
        gsap.to(this, { y: -parallaxValue / 30, duration: 1, ease: 'none', overwrite: 'auto' });
      } else if ($this.hasClass('acBoxLeft')) {
        gsap.to(this, { x: -parallaxValue / 10, duration: 1, ease: 'none', overwrite: 'auto' });
      }
    });
  }

  $(window).on('scroll', updateParallaxEffect);
  $(window).on('resize load', function () {
    gsap.set(parallaxElements, { clearProps: 'all' });
